    ...
    <CardContent className="p-4">
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 mb-3">
        <div className="text-center">
          ...
          <div className="font-mono text-3xl font-semibold tabular text-fg leading-none mt-1">
            {homeScore}
          </div>
        </div>
      ...
      <CardContent className="p-4">
        <header className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            {match.date && (
              <span className="text-[11px] text-muted-fg">{match.date}</span>
            )}
            {match.competition && (
              <Badge tone="primary">{match.competition}</Badge>
            )}
          </div>
            ...
            {resultKey && (
              <span
                className={cn(
                  ...
                  resultStyles[resultKey],
                )}
                aria-label={
                  resultKey === 'W' ? 'Victoria' :
                  resultKey === 'D' ? 'Empate' : 'Derrota'
                }
              >
                {resultKey}
              </span>
            )}
            <Badge tone="goal">Final</Badge>
          </div>
        </header>
        ...
        <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 mb-4">
          <TeamLine
            ...
            dimmed={!homeWon && match.hs !== match.as}
          />
          ...
          <TeamLine
            ...
            dimmed={!awayWon && match.hs !== match.as}
            align="right"
          />
        </div>
...
const TeamLine = ({
  ...
  <div className={cn('min-w-0', align === 'right' ? 'text-right' : 'text-left')}>
    <div
      className={cn(
        'flex items-center gap-2 text-xs font-medium text-fg truncate',
    ...
    <div
      className={cn(
        'font-mono text-2xl font-semibold tabular leading-none mt-1',
        dimmed ? 'text-muted-fg' : 'text-fg',
      )}
    >
      {score}
    </div>
